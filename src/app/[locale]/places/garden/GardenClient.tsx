...
import garden from "@/data/DataGarden";
// المكون يستقبل البيانات والترجمات كـ props
function GardenClient({ translations }: { translations: Translations }) {
  const { locale } = useParams();
  const [regionFilter, setRegionFilter] = useState<string>("All");
  const [categoryFilter, setCategoryFilter] = useState<string>("All");

  const filteredGarden = garden.filter(
    (g) =>
      (regionFilter === "All" || g.region === regionFilter) &&
      (categoryFilter === "All" || g.category === categoryFilter)
  );

  const uniqueRegions = ["All", ...new Set(garden.map((d) => (locale === 'ar' ? d.region : d.regionEn)))];
  const uniqueCategories = [
    "All",
    ...new Set(garden.map((d) => (locale === 'ar' ? d.category : d.categoryEn))),
  ];
