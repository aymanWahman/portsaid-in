...
import cloth from "@/data/DataClothingStores";
// المكون يستقبل البيانات والترجمات كـ props
function ClothClient({ translations }: { translations: Translations }) {
  const { locale } = useParams();
  const [regionFilter, setRegionFilter] = useState<string>("All");
  const [categoryFilter, setCategoryFilter] = useState<string>("All");

  const filteredCloth = cloth.filter(
    (c) =>
      (regionFilter === "All" || c.region === regionFilter) &&
      (categoryFilter === "All" || c.category === categoryFilter)
  );

  const uniqueRegions = ["All", ...new Set(cloth.map((d) => (locale === 'ar' ? d.region : d.regionEn)))];
  const uniqueCategories = [
    "All",
    ...new Set(cloth.map((d) => (locale === 'ar' ? d.category : d.categoryEn))),
  ];
