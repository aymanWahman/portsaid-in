...
import Pharmacies from '@/components/DataPharmacies';
...
const pharmacies: React.FC = () => {
  ...
  const uniqueRegions = ['All', ...new Set(Pharmacies.map((d) => d.region))];
  const uniqueCategories = ['All', ...new Set(Pharmacies.map((d) => d.category))];
